...
from ocr.core.utils import get_contour_boxes, get_img_from_box, get_threshold_img,get_threshold_img_hsv, find_max_box, show_img, draw_rec, plot_img
from ocr.core.resize import resize_img_by_height, resize_img_by_width
#---------------------------
...
def get_main_text(img, box, kernel_height):
    ...
    thresh = get_threshold_img_hsv(img, kernel)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (10, 10))
    morph = cv2.morphologyEx(thresh, cv2.MORPH_DILATE, kernel)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 15))
    morph = cv2.morphologyEx(morph, cv2.MORPH_OPEN, kernel)
    contour_boxes = get_contour_boxes(morph)
    max_box = max(contour_boxes, key=lambda tup: tup[0] * tup[1])

    x, y, w, h = max_box
    return (x0+x, y0+y, x0+x+w, y0+y+h)

def get_main_text_name(img, box, kernel_height):
    x0, y0, x1, y1 = box
    img = img[y0:y1, x0:x1]
...
def get_name(img, box):
    ...
    height, width, _ = img.shape
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (25, 25))
    thresh_img = get_threshold_img_hsv(img, kernel)
...
def detect_info(img):
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 15))
    threshold_img = get_threshold_img_hsv(label_img, kernel)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (label_img.shape[1] // 2, 5))
    morph = cv2.dilate(threshold_img, kernel, iterations=1)
    contour_boxes = get_contour_boxes(morph)
    ...
    cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\number.png",number_img)
    # get name part
    name_box = info_list[0]
    name_box = get_name(img, get_main_text_name(img, name_box, 5))
    name_img = get_img_from_box(orig, ratio, name_box, padding=2)
    name_img = cut_blank_part(name_img)
    cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\name.png", name_img)
    # get dob part
    dob_box = info_list[1]
    dob_box = get_name(img, get_main_text_name(img, dob_box, 5))
    dob_img = get_img_from_box(orig, ratio, dob_box)
    dob_img = cut_blank_part(dob_img)
    cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\dob.png", dob_img)
    #get country part
    country_box = info_list[2]
    x, y, x1, y1 = country_box
    last_y = dob_box[-1]
    country_img = process_result(orig, ratio, get_two_lines_img(img, (x, last_y, x1, y1)))[0]
    ...
    if len(country_img_list) == 1:
        cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\country.png", country_img_list[0])
    else:
        cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\country1.png", country_img_list[0])
        cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\country2.png", country_img_list[1])
    #get address part
    address_box = info_list[3]
    ...
    if len(address_img_list) == 1:
        cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\address.png", address_img_list[0])
    else:
        cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\address1.png", address_img_list[0])
        cv2.imwrite("C:\\Users\\kinhk\\PycharmProjects\\DoAnTotNgiep\\app\\static\\ocrid_images\\address2.png", address_img_list[1])
    return face, number_img, name_img, dob_img, country_img, address_img, country_img_list, address_img_list
